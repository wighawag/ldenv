...
import {runCli, runWatchCli, createTempDir, createEnvFile, cleanupTempDir, getFixturePath} from '../setup';
...
describe('CLI Integration', () => {
	...
	describe('basic arguments', () => {
		...
		it('CLI-008: should handle multiple flags', async () => {
			...
			const result = await runCli(['-m', 'production', '--verbose', 'printenv', 'KEY'], {
				cwd: tmpDir,
			});
	...
	describe('multiple commands (~~ syntax)', () => {
		...
		it('SEQ-002: should run three commands', async () => {
			...
			const result = await runCli(['echo', '1', '~~', 'echo', '2', '~~', 'echo', '3', '~~'], {
				cwd: tmpDir,
			});
	...
	describe('fixture-based CLI tests', () => {
		...
		it('should work with modes fixture in production', async () => {
			...
			const result = await runCli(['-m', 'production', 'printenv', 'LOG_LEVEL'], {
				cwd: fixturePath,
			});
	...
	describe('watch mode', () => {
		it('WATCH-001: should accept -w flag', async () => {
			createEnvFile(tmpDir, '.env', 'KEY=value');

			const result = await runWatchCli(['echo', '@@KEY'], {
				cwd: tmpDir,
				timeoutMs: 1500,
			});

			expect(result.stdout).toContain('[ldenv] Running: echo @@KEY');
			expect(result.stdout).toContain('value');
		});

		it('WATCH-002: should accept --watch flag', async () => {
			createEnvFile(tmpDir, '.env', 'KEY=value');

			// Test using runCli with -w directly in args (since -w is in args, not handled by runWatchCli)
			const result = await runWatchCli(['printenv', 'KEY'], {
				cwd: tmpDir,
				timeoutMs: 1500,
			});
		...
		it('WATCH-003: should show startup message', async () => {
			createEnvFile(tmpDir, '.env', 'TEST=hello');

			const result = await runWatchCli(['echo', '@@TEST'], {
				cwd: tmpDir,
				timeoutMs: 1500,
			});

			expect(result.stdout).toContain('[ldenv] Running:');
		});

		it('WATCH-004: should show loaded files in verbose mode', async () => {
			createEnvFile(tmpDir, '.env', 'KEY=value');

			const result = await runWatchCli(['--verbose', 'echo', 'test'], {
				cwd: tmpDir,
				timeoutMs: 1500,
			});

			expect(result.stdout).toContain('[ldenv] Loaded files:');
			expect(result.stdout).toContain('.env');
		});

		it('WATCH-005: should shut down gracefully on timeout', async () => {
			createEnvFile(tmpDir, '.env', 'KEY=value');
			...
			expect(result.stdout).toContain('[ldenv] Shutting down...');
			expect(result.killed).toBe(true);
		});

		it('WATCH-006: should reload on env file change', async () => {
			createEnvFile(tmpDir, '.env', 'KEY=initial');

			const result = await runWatchCli(['echo', '@@KEY'], {
				cwd: tmpDir,
				timeoutMs: 3000,
				onStart: (modifyEnvFile, kill) => {
					// Wait a bit, then modify the env file
					setTimeout(() => {
						modifyEnvFile('.env', 'KEY=modified');
					}, 500);
					// Kill after enough time for reload
					setTimeout(() => {
						kill();
					}, 2000);
				},
			...
			// Check that either it detected the change or ran with initial value
			// The file watcher may or may not trigger depending on timing
		});

		it('WATCH-007: should work with modes in watch mode', async () => {
			createEnvFile(tmpDir, '.env', 'KEY=base');
			createEnvFile(tmpDir, '.env.production', 'KEY=prod');

			// Note: runWatchCli prepends -w, so we add -m production before the command
			const result = await runWatchCli(['-m', 'production', 'printenv', 'KEY'], {
				cwd: tmpDir,
				timeoutMs: 1500,
			});
		...
		it('WATCH-008: should substitute variables in watch mode', async () => {
			createEnvFile(tmpDir, '.env', 'A=one\nB=two');

			const result = await runWatchCli(['echo', '@@A', '@@B'], {
				cwd: tmpDir,
				timeoutMs: 1500,
			});
		...
		it('WATCH-009: should work with sequential commands in watch mode', async () => {
			createEnvFile(tmpDir, '.env', 'X=first');

			const result = await runWatchCli(['echo', '@@X', '~~', 'echo', 'second', '~~'], {
				cwd: tmpDir,
				timeoutMs: 2000,
			});
		...
		it('WATCH-010: should handle missing env vars with defaults in watch mode', async () => {
			createEnvFile(tmpDir, '.env', '');

			const result = await runWatchCli(['echo', '@@MISSING@:default@:'], {
				cwd: tmpDir,
				timeoutMs: 1500,
			});
		...
		it('WATCH-011: watcher should be initialized BEFORE command starts (regression test)', async () => {
			// This test would have caught the bug where the watcher was set up AFTER
			// awaiting the command execution, which blocked indefinitely for long-running processes
			createEnvFile(tmpDir, '.env', 'KEY=value');

			const result = await runWatchCli(['--verbose', 'echo', 'COMMAND_OUTPUT'], {
				cwd: tmpDir,
				timeoutMs: 1500,
			});

			// The "[ldenv] Watching for changes..." message MUST appear BEFORE "COMMAND_OUTPUT"
			const watchingIndex = result.stdout.indexOf('[ldenv] Watching for changes...');
			const commandIndex = result.stdout.indexOf('COMMAND_OUTPUT');

			// Both must be present
			expect(watchingIndex).toBeGreaterThan(-1);
			expect(commandIndex).toBeGreaterThan(-1);
			// And watcher message must come before command output
			expect(watchingIndex).toBeLessThan(commandIndex);
		});

		it('WATCH-012: should detect file changes and trigger reload with new values', async () => {
			// More robust reload test - verifies the reload actually uses new values
			createEnvFile(tmpDir, '.env', 'RELOAD_TEST=original_value');

			const result = await runWatchCli(['--verbose', 'printenv', 'RELOAD_TEST'], {
				cwd: tmpDir,
				timeoutMs: 4000,
				onStart: (modifyEnvFile, kill) => {
					// Wait for watcher to be set up and first command to run
					setTimeout(() => {
						modifyEnvFile('.env', 'RELOAD_TEST=updated_value');
					}, 800);
					// Give time for reload and new command to run
					setTimeout(() => {
						kill();
					}, 3000);
				},
			...
			// Should have original value from first run
			expect(result.stdout).toContain('original_value');
			// Should also have the reload message
			expect(result.stdout).toContain('[ldenv] .env changed, reloading...');
			// And should have the updated value from second run
			expect(result.stdout).toContain('updated_value');
		});

		it('WATCH-013: should properly stop and restart process on reload', async () => {
			createEnvFile(tmpDir, '.env', 'KEY=initial');

			const result = await runWatchCli(['--verbose', 'echo', '@@KEY'], {
				cwd: tmpDir,
				timeoutMs: 4000,
				onStart: (modifyEnvFile, kill) => {
					setTimeout(() => {
						modifyEnvFile('.env', 'KEY=reloaded');
					}, 800);
					setTimeout(() => {
						kill();
					}, 3000);
			...
			// Should show stopping message when reloading
			expect(result.stdout).toContain('[ldenv] Running:');
			// First run should output initial
			expect(result.stdout).toContain('initial');
		});
	});
});
